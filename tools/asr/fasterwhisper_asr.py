    ...
    if os.path.exists(output_file_path):
        with open(output_file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not line:
                    continue
                processed_files.add(line.split("|", 1)[0])
        print(f"Resuming ASR: {len(processed_files)} already processed files found in {output_file_path}")
            ...
            try:
                ...
                if text == "":
                    for segment in segments:
                        text += segment.text
                line = f"{file_path}|{output_file_name}|{info.language.upper()}|{text}"
                f.write(line + "\n")
                f.flush()
